...
if [ "$BITS" = 32 ]; then
	echo -e "Your system architecture is $ARCH which is unsupported to run WizBot. \nYour OS: $OS \nOS Version: $VER"
	echo
	printf "\e[1;31mPlease check the WizBot self-hosting guide for alternatives.\e[0m\n"
	rm wizbotpm2setup.sh
	exit 1
fi


echo -e "Welcome to WizBot's pm2 setup! \nWould you like to continue? \nYour OS: $OS \nOS Version: $VER \nArchitecture: $ARCH"

while true; do
    read -p "[y/n]: " yn
    case $yn in
        [Yy]* ) clear; echo Running WizBot pm2 Setup; sleep 2; break;;
        [Nn]* ) echo Quitting...; rm wizbotpm2setup.sh && exit;;
...
echo
echo "WizBot pm2 Installation completed..."
read -n 1 -s -p "Press any key to continue to the main menu..."
sleep 2

cd "$root"
rm "$root/wizbotpm2setup.sh"
exit 0